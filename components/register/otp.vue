  ...
  <v-card flat max-width="450" class="rounded-lg px-md-4 py-2">
    <v-card-title class="d-flex flex-column justify-center font-weight-light text-subtitle-1 text-md-h6 my-2">
      ...
      <div class="text-subtitle-1 text1--text text-center my-1 font-weight-light ">
        OTP has been sent to <span class="font-weight-bold">{{ registration && registration.phoneNumber | hideNumber }}</span>
      </div>
    </v-card-title>
...
export default {
  ...
  methods: {
    ...mapActions({ register: 'authentication/register', sendOtp: 'authentication/sendOtp' }),
    inputValue (otp) {
      this.otp = otp
    },
    submit () {
