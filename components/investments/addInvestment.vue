<template>
  <p-side-bar :divided="false" :loading="loading.investments" :drawer="drawer" :toggle="toggle">
    <template v-slot:header>
      <div class="text-subtitle-2 primary--text text-uppercase">
            ...
            <v-row no-gutters class="ma-0">
              ...
              <v-col cols="12" class="d-flex flex-column pa-0 mb-n2 mt-2">
                <div class="d-flex align-center my-1 text-subtitle-1 text-md-h6  font-weight-medium white--text">
                  <span>
                    <span class="font-weight-light text-subtitle-2"> Min (</span>{{ selectedPlan && selectedPlan.minAmount | currency }}<span class="font-weight-light text-subtitle-2">)</span>
                  </span>
                  <v-spacer />
                  <span class="text-body-2">{{ selectedPlan && selectedPlan.duration }}</span>
                </div>
              </v-col>
            </v-row>
          </v-card-text>
            ...
            <v-row no-gutters class="ma-0">
              ...
              <v-col cols="6" class="d-flex flex-column text-center">
                ...
                <div class="secondary--text font-weight-medium text-subtitle-1">
                  {{ selectedPlan && selectedPlan.percentage }}%
                </div>
              </v-col>
...
export default {
  ...
  computed: {
    ...
    getDividend () {
      ...
      const amount = this.amount === '' ? this.selectedPlan && this.selectedPlan.minAmount : this.amount
      const interest = this.selectedPlan && this.selectedPlan.percentage
  ...
  methods: {
    ...mapActions({ initAlert: 'controller/initAlert', invest: 'admin/invest' }),
    submit () {
      if (this.$refs.form.validate()) {
        const amount = parseInt(this.pAmount.split(',').join('').slice(1))

        if (amount < (this.selectedPlan && this.selectedPlan.minAmount)) {
          this.initAlert({
            is: true,
            type: 'error',
            persistence: true,
            message: `The minimum investible amount  ₦${this.selectedPlan && (this.selectedPlan.minAmount).toLocaleString('en-NG', { style: 'currency', code: 'NG', currency: 'NGN' })}`
          })
          return false
        } else {
